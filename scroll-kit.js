...
(function() {
  var calculate_all_offsets, calculate_all_stickes, check_if_can_bottom, check_if_can_stick, check_if_can_unbottom, check_if_can_unstick, check_if_fit, debug, event_handler, group_id, html_element, initialize_sticky, last_direction, last_scroll, offsets, placeholder, refresh_all_stickies, set_classes, stack, static_interval, test_all_offsets, test_for_scroll_and_offsets, test_node_enter, test_node_exit, test_node_passing, test_node_scroll, test_on_scroll, trigger, update_everything, update_margins, update_metrics, update_offsets, update_sticky, win, win_height;
  ...
  debug.info('jump').on('change', function(e) {
    return $.scrollKit.scrollTo(e.target.selectedIndex, stack.offsetTop);
  });
  ...
  test_node_passing = function(node) {
    ...
    if (stack.gap.offset > 0) {
      ...
      if (test_top && test_bottom && (stack.gap.nearest !== node.offset.index)) {
  ...
  test_node_exit = function(node) {
    ...
    stack.visibleIndexes = stack.visibleIndexes.filter(function(old) {
  ...
  update_everything = function(destroy) {
    ...
    if (debug.is_enabled) {
      debug.info('jump').html((function() {
        var j, ref, results;
        results = [];
        for (i = j = 1, ref = stack.contentNodes.length; 1 <= ref ? j <= ref : j >= ref; i = 1 <= ref ? ++j : --j) {
  ...
  $.scrollKit = function(params, callback) {
    ...
    if (params === 'destroy') {
      ...
      if (params !== 'update') {
        ...
        if (params.top >= 0) {
          stack.offsetTop = +params.top;
        }
        if (params.gap >= 0) {
          stack.gap.offset = +params.gap;
        }
        if (debug.is_enabled) {
          debug.info('gap').css('top', stack.gap.offset >= 0 ? stack.gap.offset : -1);
        }
